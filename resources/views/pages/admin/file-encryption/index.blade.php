                ...
                >
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>{{ $file_encryption->file_size }} <span class="mx-0">Kb</span></td>
                            <td>{{ $file_encryption->information }}</td>
                            <td>
                                <!-- Button untuk membuka modal -->
                                <a class="btn btn-info decrypt-button" data-toggle="modal" data-target="#decryptModal-{{ $file_encryption->id }}" data-file-id="{{ $file_encryption->id }}">
                                    <i class="fa fa-pencil-alt"></i>
                                    <small>Decrypt File</small>
                                </a>
                                <!-- Modal Decrypt File -->
                                <div class="modal fade" id="decryptModal-{{ $file_encryption->id }}" tabindex="-1" aria-labelledby="decryptModalLabel" aria-hidden="true">
                                    <div class="modal-dialog">
                                        <div class="modal-content">
                                            <div class="modal-header">
                                                <h5 class="modal-title" id="decryptModalLabel">Decrypt File</h5>
                                                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                                    <span aria-hidden="true">&times;</span>
                                                </button>
                                            </div>
                                            <form id="decryptForm" action="{{ route('file_encryption.update', $file_encryption) }}" method="post">
                                                @csrf
                                                @method('PUT')
                                                <div class="modal-body">
                                                    <div class="form-group">
                                                        <label for="encryption_key">Password</label>
                                                        <input type="password" class="form-control" name="encryption_key" placeholder="Enter password">
                                                    </div>
                                                </div>
                                ...
                                <form  method="post" class="d-inline" action="{{ route('file_encryption.destroy', $file_encryption->id) }}" >
                                @csrf
                                @method('delete')
                                <!-- Tombol untuk memunculkan modal konfirmasi -->
                                <button type="button" class="btn btn-danger" data-toggle="modal" data-target="#confirmDeleteModal">
                                    <i class="fa fa-trash"></i><small>Delete Data</small>
                                </button>
